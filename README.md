...
(2) The **fixed point method** requires that $g([a,b])\subset [a,b]$ in addition to $g$'s continuity on $[a,b]$. If in addition $\sup |g'|_{\infty}<1$ on $[a,b]$, then the fixed point $x=p$, satisfying $g(p)=p$, is unique.  Thus, in addition to $[a,b]$, we need $g\in C([a,b])$ satisfying $g([a,b])\subset [a,b]$ and ideally $\sup |g'|_{\infty}<1$.   
